...
tasks.withType(LinkSharedLibrary::class.java).configureEach {
    linkerArgs.addAll("user32.lib", "Gdi32.lib")
}

tasks.register<Jar>("dist") {
    dependsOn("assembleReleaseX86", "assembleReleaseX86-64")
    archiveFileName.set("o2xfs-xfs-service-std-bin.jar")
    destinationDirectory.set(file("$buildDir/dist"))
    from("$buildDir/lib/main/release/x86/o2xfs-xfs-service-std-bin.dll") {
        into("x86")
    }
    from("$buildDir/lib/main/release/x86-64/o2xfs-xfs-service-std-bin.dll") {
        into("x86_64")
    }
    rename("(.+)-bin(.+)", "$1$2")
}
